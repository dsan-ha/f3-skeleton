	...
	**/
	function factory($row) {
		...
		if (isset($mapper->trigger['load']))
			F4::instance()->call($mapper->trigger['load'],$mapper);
		return $mapper;
	}
	...
	**/
	function skip($ofs=1) {
		...
		if (!$dry && isset($this->trigger['load']))
			F4::instance()->call($this->trigger['load'],$this);
		return $out;
	}
	...
	**/
	function insert() {
		...
		if (isset($this->trigger['beforeinsert']) &&
			F4::instance()->call($this->trigger['beforeinsert'],
				[$this,$pkeys])===FALSE)
			return $this;
		if ($this->valid())
			// duplicate record
        ...
        if ($fields) {
			...
			if (isset($this->trigger['afterinsert']))
				F4::instance()->call($this->trigger['afterinsert'],
					[$this,$pkeys]);
	...
	**/
	function update() {
		...
		if (isset($this->trigger['beforeupdate']) &&
			F4::instance()->call($this->trigger['beforeupdate'],
				[$this,$pkeys])===FALSE)
			return $this;
		foreach ($this->fields as $key=>$field)
			if ($field['changed']) {
		...
		if (isset($this->trigger['afterupdate']))
			F4::instance()->call($this->trigger['afterupdate'],
				[$this,$pkeys]);
	...
	**/
	function erase($filter=NULL,$quick=TRUE) {
		...
		if (isset($this->trigger['beforeerase']) &&
			F4::instance()->call($this->trigger['beforeerase'],
				[$this,$pkeys])===FALSE)
			return 0;
		$out=$this->db->
			exec('DELETE FROM '.$this->table.' WHERE '.$filter.';',$args);
		if (isset($this->trigger['aftererase']))
			F4::instance()->call($this->trigger['aftererase'],
				[$this,$pkeys]);
		return $out;
	}
