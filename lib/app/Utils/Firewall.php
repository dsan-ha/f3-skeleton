...
class Firewall
{
    ...
    {
        $f3 = F3::instance();
        $telegram_secret = $f3->g('firewall.telegram_secret',self::TELEGRAM_TOKEN_SECRET);
        $signature_interval = $f3->g('firewall.signature_interval',self::SIGNATURE_INTERVAL);
        $signature_fail_limit = $f3->g('firewall.signature_fail_limit',self::SIGNATURE_FAIL_LIMIT);
    ...
    protected function checkRateLimit(array $accessList = []): bool
    {
        ...
        $totalInPastRange = 0;
        $f3 = F3::instance();
        $rate_limit = $f3->g('firewall.rate_limit',self::RATE_LIMIT);
        $rate_interval = $f3->g('firewall.rate_interval',self::RATE_INTERVAL);
    ...
    protected function checkDos(array $accessList = []): bool
    {
        ...
        $entries = [];
        $f3 = F3::instance();
        $dos_interval = $f3->g('firewall.dos_max_hits',self::DOS_INTERVAL);
        $dos_max_hits = $f3->g('firewall.dos_max_hits',self::DOS_MAX_HITS);
    ...
    protected function block(string $reason): void
    {
        ...
        if (!in_array($this->ip, $banManager->isBannedCache)) {
            ...
            $logFile = $f3->get('log.firewall_log');
            if($logFile){
                $txt = sprintf(
